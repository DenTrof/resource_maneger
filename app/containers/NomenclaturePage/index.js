...
import { makeSelectNomenclature, makeValuesNomenclatureForm } from './selectors';
import ModalValuesForm from './Forms/ModalValuesForm';
import { submitAction } from './actions';

export class NomenclaturePage extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super(props);
  ...
  componentDidUpdate() {
    document.querySelector('#nomenclature tbody').onclick = (e) => {
      const cell = e.target;
  ...
  saveAllData = () => {
    ...
    const nomenclatureData = nomenclature.nomenclatureData[this.state.idTable];
    // console.log('eqData55555555', operationData);
    if (nomenclatureData) {
      try {
        return axios.put(`http://manufacture-service-api.dn-kronas.local/api/web/v1/nomenclature/${this.state.idEq}`,
          {
            name: nomenclatureData.name,
            articul: nomenclatureData.articul,
            link: nomenclatureData.link,
          }
        );
      } catch (error) {
        console.error(error);
  ...
  saveChangedData = () => {
    ...
    const formData = valuesForm.NomenclatureForm.values;
    const equData = this.props.nomenclature.nomenclatureData;
    ...
    return savedata(nomenclature);
  }

  сustomDeleteButton = (cell, row) => {
    return (<button
  ...
  customConfirm = (next, row) => {
    ...
    if (confirm('ВЫ ДЕЙСТВИТЕЛЬНО ХОТИТЕ УДАЛИТЬ ЭТУ ЗАПИСЬ ?')) {
      next();
      try {
        return axios.delete(`http://manufacture-service-api.dn-kronas.local/api/web/v1/nomenclature/${row[0]}`);
      } catch (error) {
        console.error(error);
      }
    }
  ...
  render() {
    ...
    const { nomenclature } = this.props;
    const nomenclatureTypes = nomenclature.nomenclatureTypes.map((item) => item.name);
    return (
      <div id="nomenclature">
        <div className="animated fadeIn">
          <Card>
            <CardHeader>Справочник Номенклатура</CardHeader>
            <CardBody>
              <NomenclatureModal
              ...
              <BootstrapTable
                ...
                data={nomenclature.nomenclatureData}
                striped
                hover
                pagination
                options={options}
              ...
              >
                ...
                <TableHeaderColumn
                  ...
                  dataSort
                  filter={{ type: 'TextFilter' }}
                  // editable={{ type: 'select', options: { values: nomenclatureTypes } }}
                >Наименование
                </TableHeaderColumn>
                <TableHeaderColumn
                  dataField="articul"
...
class NomenclatureModal extends React.Component {
  constructor(props) {
    super(props);
...
function mapDispatchToProps(dispatch) {
  return {
    savedata: (nomenclature) => dispatch(submitAction(nomenclature)),
  };
}

const mapStateToProps = createStructuredSelector({
  nomenclature: makeSelectNomenclature(),
  valuesForm: makeValuesNomenclatureForm(),
});


const withConnect = connect(mapStateToProps, mapDispatchToProps);
const withReducer = injectReducer({ key: 'nomenclature', reducer });
const withSaga = injectSaga({ key: 'nomenclature', saga });
