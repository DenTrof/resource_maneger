...
import { makeSelectOperation, makeValuesOperationForm } from './selectors';
import ModalValuesForm from './Forms/ModalValuesForm';
import { submitAction } from './actions';

export class OperationPage extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super(props);
  ...
  componentDidUpdate() {
    document.querySelector('#group_operations tbody').onclick = (e) => {
      const cell = e.target;
  ...
  saveAllData = () => {
    ...
    const operationData = operation.operationData[this.state.idTable];
    // console.log('eqData55555555', operationData);
    if (operationData) {
      try {
        return axios.put(`http://manufacture-service-api.dn-kronas.local/api/web/v1/group-operations/${this.state.idEq}`,
          {
  ...
  saveChangedData = () => {
    ...
    const equData = this.props.operation.operationData;
    const eqData = equData[equData.length - 1].id;
    const operation = {
      id: +eqData + 1,
      name: `${valuesForm.OperationForm.values.name}`,
    };
  ...
  customConfirm = (next, row) => {
    if (confirm('ВЫ ДЕЙСТВИТЕЛЬНО ХОТИТЕ УДАЛИТЬ ЭТУ ЗАПИСЬ ?')) {
      next();
      try {
        return axios.delete(`http://manufacture-service-api.dn-kronas.local/api/web/v1/group-operations/${row[0]}`);
      } catch (error) {
        console.error(error);
      }
    }
  ...
  render() {
    ...
    const { operation } = this.props;
    return (
      <div id="group_operations">
        <div className="animated fadeIn">
          <Card>
            <CardHeader>Справочник Группа Операций</CardHeader>
            <CardBody>
              <OperationModal
...
function mapDispatchToProps(dispatch) {
  return {
    savedata: (operation) => dispatch(submitAction(operation)),
  };
}

const mapStateToProps = createStructuredSelector({
  operation: makeSelectOperation(),
  valuesForm: makeValuesOperationForm(),
});


const withConnect = connect(mapStateToProps, mapDispatchToProps);
const withReducer = injectReducer({ key: 'operation', reducer });
const withSaga = injectSaga({ key: 'operation', saga });
